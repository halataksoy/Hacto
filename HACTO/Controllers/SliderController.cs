    ...
    public class SliderController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "SliderId,Baslik,Aciklama,ResimURL")] Slider slider, HttpPostedFileBase ResimURL)
        {
            if (ModelState.IsValid)
            {
                if (ResimURL != null)
                {
                    ...
                    string sliderimgname = Guid.NewGuid().ToString() + imginfo.Extension;//resimi isimlendiriyoruz
                    img.Resize(1024, 360);//logo boyutunu ayarlar width height
        ...
        public ActionResult Edit([Bind(Include = "SliderId,Baslik,Aciklama,ResimURL")] Slider slider, HttpPostedFileBase ResimURL, int id)
        {
            if (ModelState.IsValid)
            {
                ...
                if (ResimURL != null)
                {
                    if (System.IO.File.Exists(Server.MapPath(s.ResimURL)))//KİMLİK LOGO URLLİ VAR MI ONA BAKICAK 
                    {
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            if (System.IO.File.Exists(Server.MapPath(slider.ResimURL)))//slider var mı ona bakıcka 
            {
